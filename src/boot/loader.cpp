...
// the stock OTA. Once running it uses deskmate's normal 4m1m flash layout,
...
// the repo. Without them it falls back to an open "DeskMate-Loader" AP at
...
void setup() {
  ...
  if (!joined) {
    WiFi.mode(WIFI_AP);
    WiFi.softAP("DeskMate-Loader");        // open AP -> browse to 192.168.4.1/update
  }
